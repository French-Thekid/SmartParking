...
@Component({
  selector: 'app-sspot-r',
  templateUrl: './sspot-r.page.html',
  styleUrls: ['./sspot-r.page.scss'],
})
export class SspotRPage implements OnInit {
  query: Query;
    ...
    if (this.staffCheck == true) {
      ...
      var subscripton2 = snapshotResult2.subscribe(async doc => {
        ...
        if ((this.s_space2.status == false) || (this.s_space2.reserved == true)) {
          ...
          this.afstore.collection('reports').doc(JSON.parse(localStorage.getItem("License"))+"").set({
            reporterLicNbr: JSON.parse(localStorage.getItem('License'))+"",
            reportLicNbr2: JSON.parse(localStorage.getItem('O_L'))+"",
            report: JSON.parse(localStorage.getItem('TR'))+"",
            newSpot: this.selectedSpot
          });
          this.reportMade();
